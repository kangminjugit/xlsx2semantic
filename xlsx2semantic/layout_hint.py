...
class TableLayoutHint:
    ...
    def create(
        ...
        if not has_title and not has_header and not has_meta:
            return None
        ...
        if has_meta:
            m = _COL_ONLY.match(row_meta_col.strip().upper())
            if m:
                meta_col = _col_to_num(m.group(1))
    ...
    def resolve_defaults(self, max_row: int, max_col: int) -> TableLayoutHint:
        ...
        return TableLayoutHint(
            ...
            row_meta_col_num=self.row_meta_col_num,
        )
    ...
    def has_row_meta_col(self) -> bool:
        return self.row_meta_col_num > 0
